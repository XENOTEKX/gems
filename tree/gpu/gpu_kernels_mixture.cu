...
// postorder + one preorder sweep yields df/ddf for every edge, vs a per-edge two-sub-root
// split). Rooted at an internal node `root`; out_df[v]/out_ddf[v] hold d(lnL)/db_v and the 2nd derivative for
