...
// ============================================================================================================
// Single-edge branch derivative for profile mixtures: df/ddf summed over the N*ncat regimes, cross-checked vs CPU
// computeLikelihoodDerv. Combines the two-sub-root central-edge split (gpuComputeEdgeDervCleanRoom) with the
// per-class eigen assembly (gpuComputeTreeLnLCleanRoomMix). Returns df = d(lnL)/dt (un-negated, like the
// single-model path; computeFuncDerv negates downstream); *out_ddf the 2nd derivative; *out_lnL the tree lnL at t.
// Eligibility gate is identical to the mixture lnL path (+I/fused/PMSF/nonrev/single-model -> NaN -> CPU) so an
// edge never gets a GPU lnL but a CPU derivative or vice versa.
// ============================================================================================================
double PhyloTree::gpuComputeEdgeDervCleanRoomMix(PhyloNeighbor *dad_branch, PhyloNode *dad, double *out_ddf, double *out_lnL) {
    ...
    if (site_rate->getPInvar() > 0.0) return (double)NAN;  // +I omitted in this sweep -> CPU
    int ncat = site_rate->getNRate();
    int nptn = (int)aln->size();
    int ntax = (int)aln->getNSeq();
    if (ncat < 1) return (double)NAN;
    ...
    // ---- per-class eigen (component pointers handle the AVX padding), freq, per-regime weight = w_m*catProp_c ----
    std::vector<double> Uinv((size_t)N*ns*ns), UinvRowSum((size_t)N*ns), freqC((size_t)N*ns);
    std::vector<double> evalC((size_t)N*ns), Uc((size_t)N*ns*ns);
    std::vector<double> catRate(ncat), catProp(ncat), wreg((size_t)R);
    for (int c = 0; c < ncat; c++) { catRate[c] = site_rate->getRate(c); catProp[c] = site_rate->getProp(c); }
    ...
    // ---- two-sub-root DFS, central edge (node<->dadN) excluded from both (same as single-model derivative) ----
    map<Node*,int> nid; vector<Node*> nodes; vector<double> parentLen; vector<int> isLeafV, leafTax;
    function<void(Node*,Node*,double)> indexDfs = [&](Node *n, Node *par, double lenToPar) {
...
double PhyloTree::optimizeParametersJOLT(int fixed_len, bool brlenOnly, bool leanTail, int brlenMaxIter) {
    ...
    // ncat<=JOLT_FREERATE_MAXCAT, full model-param path only (brlenOnly/lean holds +R fixed).
    ...
    // --jolt-diag: time the once-per-call host rebuild (DFS reindex + O(ntax*nptn) tip[] recompaction + flat arrays)
    ...
    double jd_h1 = params->jolt_diag ? getRealTime() - _jd_h1_t0 : 0.0;   // --jolt-diag: end host-rebuild timing
    ...
    double joltLnL = gpu_jolt_optimize(ns, nptn, ncat, ntax, nNodes, /*root=*/nid[R],
        ...
        nFreeQ, (nFreeQ > 0 ? q0vec.data() : nullptr), jolt_qdecompose_intree, &qctx,   // DNA free-Q
